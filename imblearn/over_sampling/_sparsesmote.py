...
class SparseBaseSMOTE(BaseOverSampler):
    ...
    def __init__(self,
                 ...
                 n_jobs=1,
                 ratio=None):
        super(SparseBaseSMOTE, self).__init__(
        ...
        if sparse.issparse(X):
            ...
            for i, (row, col, step) in enumerate(zip(rows, cols, steps)):
                if X[row].nnz:
                    ...
                    num_instances += 1
                    for i in range(1, int(math.floor(sample_dataset_ratio)), 1):
...
SparseSMOTE # doctest: +NORMALIZE_WHITESPACE
    ...
    def __init__(self,
        ...
        SparseBaseSMOTE.__init__(self, sampling_strategy=sampling_strategy,
